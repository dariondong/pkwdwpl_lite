...
**一个信标只占一行**，固定列布局、所有字段同一字号 12（呼号不加粗）。
列表内的字号缩放**封顶 1.15 倍**（`AppTheme.maxListTextScale`）——
固定列布局对字号很敏感，用户在系统里把字体调到 1.5x~2x 时，
右侧的距离/时间会被挤没，所以刻意让列表不跟随到那么大。

```
 呼号 图标 方向   距离       时间
 BG1UBU-9 🚗  ↑   50.0 km   10:23
```

* **呼号**：等宽字体，吃掉剩余宽度；
* **设备图标紧挨方向箭头**（间隔约 4px）—— 图标跟在呼号后面、方向在右侧时
  中间会空出一大截，所以改成「图标 + 箭头」成组放在一起；
* **方向**：只画一个按航向旋转的**箭头，不显示度数**（0° = 正北）；无航向显示 `--`；
* **距离**：菜单里一键 **KM ⇄ MI**；若设置了本机参考坐标则用 Haversine 实算，
  否则显示报文第 11 字段；
* **接收时间**：默认 `HH:mm`（本地时区）；菜单里可切「时间显示秒」。
  默认不显示秒能给呼号腾出宽度，是小屏 + 大档位下不溢出的关键；
* **单击 → 详情页**；**双击 → 离线地图并把该台站居中**；
* 列表上方只有**一行**很轻的状态条：连接状态点（绿=已连）+ 台站数 + 最后更新时间。

> 界面约束**有自动化测试守护**：`test/station_row_layout_test.dart` 会在
> 「屏宽 × 系统字号 × 缩放档位」共 7 种组合（含 320dp 小屏 + 特大档这种最严苛情形）下断言
> 「行高固定、不得因换行被撑高」「不显示接收序号」「**图标与方向箭头间隔不得过大**」
> 「所有列字号一致且不加粗」「不受系统字号影响」「切换距离单位后仍单行」。
>
> 列宽不是猜的：`AppTheme.ListMetrics` 里那组数字是用脚本算过
> 12 种「屏宽 × 档位」组合的（最严苛下呼号仍有 8.9 个字符位）。
>
> 改界面时建议先渲染预览图肉眼核对：
> `flutter test test/tools/ui_preview.dart` → `build/ui_preview/list_page.png`
> 想验证极端字号：`flutter test test/tools/ui_stress_preview.dart`
> → `build/ui_preview/stress_*.png`（多组「屏宽 × 字号」对比图）
...
| 界面缩放 | 小 / 标准 / 大 / 特大 四档循环（`AppTheme.ListScale`）；列表**不跟随系统字号**，只按这个档位缩放，字号与列宽永远同步 |
| 时间显示秒 | 开 / 关（关 = `HH:mm`，更省宽度）|
